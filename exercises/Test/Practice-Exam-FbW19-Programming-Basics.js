/**                  // fbw19 - 21th of October 2019 - Boehm, Bernd
...
const one = integer => {
    ...
    // let isDivByHundred = Boolean;

    // if (integer % 100 === 0) {
    //     isDivByHundred = true;
    // } else {
    //     isDivByHundred = false;
    // }
    // return isDivByHundred;

    return integer % 100 === 0; // easier :D
};
...
const three = integers => {
    let sumOfInt = 0;
    for (i = 0; i < integers.length && 3; i += 1) {
        sumOfInt += integers[i];
    }
    return sumOfInt;
};
...
const five = (firstAngle, secondAngle) => {
    ...
    const addAngle = firstAngle + secondAngle;
    // console.log(addAngle);
    if (addAngle <= 1 || addAngle >= 179) {
        return 'Wrong number.' // e.g. 0 or 180
    } else if (addAngle < 90) {
...
// console.log(`Input: 0, 180\t`, five(0, 180));
...
const six = (integers, callback) => {   // [1,2,3,4] , value => Math.pow(value, 2)

    // const myNewArray = [];
    // let temp = 0;

    // for (i = 0; i < integers.length; i++) {
    //     temp = callback(integers[i]);
    //     // console.log(temp);
    //     myNewArray.push(temp);
    // }
    // return myNewArray;
...
const seven = integers => {
    ...
    // const myNewArray = [];
    // let temp = 0;
    const callback = value => Math.round(Math.sqrt(value) * 100) / 100;
...
class Podcaster {
    ...
    boast() {
        ...
        return `"I'm ${splitName[0]} and ${this.podcastTitle} is the best podcast ever!"`
    }
}
